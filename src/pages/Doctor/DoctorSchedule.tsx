...
function buildInitialSchedule(availableDays: string[], timeSlots: string[]): Record<string, DaySchedule> {
  const result: Record<string, DaySchedule> = {};
  for (const day of DAYS) {
    result[day] = {
      active: availableDays.includes(day),
      slots: availableDays.includes(day) ? timeSlots : [],
    };
  }
  return result;
}
...
export default function DoctorSchedule() {
  const { currentDoctor, reloadClinic } = useClinic();

  const [schedule, setSchedule] = useState<Record<string, DaySchedule>>(() =>
    buildInitialSchedule(currentDoctor.availableDays ?? [], currentDoctor.timeSlots ?? [])
  );
  ...
  const handleSave = async () => {
    ...
    // Build availableDays and combined timeSlots from schedule
    const activeDayNames = DAYS.filter((d) => schedule[d].active);
    const allSlots = [...new Set(
      activeDayNames.flatMap((d) => schedule[d].slots)
    )].sort();

    try {
      await apiRequest(`/doctors/${currentDoctor.id}/schedule`, {
        method: 'PATCH',
        body: JSON.stringify({ availableDays: activeDayNames, timeSlots: allSlots }),
      });
      await reloadClinic();
      setSaved(true);
      setTimeout(() => setSaved(false), 2500);
    } catch (err) {
      setSaveError(err instanceof Error ? err.message : 'Unable to save schedule.');
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3 self-start">
          {saveError && <p className="text-red-500" style={{ fontSize: '0.82rem' }}>{saveError}</p>}
          <button
            onClick={() => void handleSave()}
            disabled={saving}
            className={`flex items-center gap-2 px-6 py-3 rounded-xl hover:shadow-lg transition-all disabled:opacity-60 ${saved ? 'bg-green-500 text-white' : 'bg-gradient-to-r from-emerald-600 to-teal-500 text-white'}`}
            style={{ fontWeight: 600 }}
          >
            {saved ? <><CheckCircle size={18} /> Saved!</> : saving ? <><Clock size={18} /> Saving...</> : <><Save size={18} /> Save Schedule</>}
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          { label: "Today's Slots", value: schedule[DAYS[new Date().getDay() === 0 ? 6 : new Date().getDay() - 1]]?.slots.length || 0, icon: CheckCircle, bg: 'bg-purple-50', text: 'text-purple-600' },
