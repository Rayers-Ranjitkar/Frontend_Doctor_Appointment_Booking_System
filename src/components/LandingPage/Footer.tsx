          ...
          <div className="grid md:grid-cols-4 gap-8 mb-8">
            <div>
              <div className="flex items-center gap-2 mb-4">
                ...
                <span className="text-white" style={{ fontWeight: 700, fontSize: '1.1rem' }}>MediBook</span>
              </div>
              <p style={{ fontSize: '0.85rem', lineHeight: 1.7 }}>Your trusted healthcare companion. Book appointments, manage health records, and connect with specialists.</p>
            </div>
            {[
              ...
              <div key={col.title}>
                <h4 className="text-white mb-4" style={{ fontWeight: 600, fontSize: '0.95rem' }}>{col.title}</h4>
                <ul className="space-y-2">
                  {col.links.map((link) => (
                    <li key={link}><a href="#" className="hover:text-white transition-colors" style={{ fontSize: '0.85rem' }}>{link}</a></li>
                  ))}
                </ul>
              </div>
            ))}
